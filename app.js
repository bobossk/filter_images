...
const activateLightBox = (thisElmt) => {
  ...
  // 함수가 실행되면 모든 요소 이미지의 요소가 active 클래스 삭제
  ...
  lightBox.classList.add("show"); // 라이트 박스의 show  클래스를 추가하여 활성화
  overlay.classList.add("show"); // 오버레이에 show  클래스 추가하여 활성화
};

// 이미지 요소를 순회하여 각각을 클릭했을 때 activelight  함수 실행 : 클릭한 대상 요소 파라미터로 전달
images.forEach((img) => {
...
// 이미지 요소 배열을 순회한다. 첫번째 파라미터는 이미지 요소 배열, 두번째 파라미터는 boolean 값 - true일 경우 ? 뒤에 :을 기준으로 앞에 로직이 적용되고,  false일 경우 로직이 뒤로 적용된다.
...
function showItems(thisItem) {
  ...
  // 클릭한 탭 버튼 요소의 data-filter 속성값과 이미지 요소의 data-filter 속성 값이 일치하는 것만 필터링
